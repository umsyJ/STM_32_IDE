 ...
 * Auto-generated by STM32 Block IDE  [FreeRTOS / CMSIS-RTOS v2].
 * DO NOT EDIT BY HAND - your changes will be overwritten on the next build.
 *
 * Board:    {board_name}
 * Step:     {step_ms} ms
 ...
 * Scheduling strategy
 * -------------------
 * A single "ModelTask" runs at osPriorityNormal.  osDelayUntil() suspends the
 * task for exactly {step_ms} ms between iterations, giving the FreeRTOS idle
 * task CPU time to manage heap / timers.  This is more power-efficient than
 * the bare-metal flag-polling loop and handles step overruns gracefully
 * (the task simply fires at the next deadline rather than spinning).
 */
...
static void model_step(void);
static void scope_emit(void);
static void ModelTask(void *argument);

UART_HandleTypeDef huart2;

/* ------------------------------------------------------------------- */
/* FreeRTOS task descriptor                                            */
/* ------------------------------------------------------------------- */
...
int main(void) {{
    ...
    osKernelInitialize();
    modelTaskHandle = osThreadNew(ModelTask, NULL, &modelTask_attributes);
    osKernelStart();
...
static void ModelTask(void *argument) {{
    ...
    const uint32_t period_ticks = {step_ms};  /* 1 RTOS tick = 1 ms */
 ...
 * Tuned for STM32F446RE (Cortex-M4F) @ 180 MHz with CMSIS-RTOS v2.
...
/* Map FreeRTOS ISR names to STM32 HAL names -------------------------------- */
#define vPortSVCHandler     SVC_Handler
#define xPortPendSVHandler  PendSV_Handler
/* SysTick_Handler is provided by cmsis_os2.c via osSystickHandler;
 * define it here so no duplicate symbol is created. */
...
MAKEFILE_RTOS_TEMPLATE = r"""# Auto-generated Makefile for STM32 Block IDE  [FreeRTOS build].
# Requires:
#   - arm-none-eabi-gcc (GNU Arm Embedded Toolchain)
#   - STM32CubeF4 firmware package (HAL drivers + CMSIS + FreeRTOS middleware)
#   - st-flash (from stlink-tools), only for `make flash`
#
# Set CUBE_F4 to the root of your STM32CubeF4 install, e.g.:
#   Windows cmd:  set CUBE_F4=C:/STM32Cube/STM32Cube_FW_F4_V1.28.0
...
HAL_DIR  = $(CUBE_F4)/Drivers/STM32F4xx_HAL_Driver
CMSIS    = $(CUBE_F4)/Drivers/CMSIS
RTOS_DIR = $(CUBE_F4)/Middlewares/Third_Party/FreeRTOS/Source
CMSIS_OS = $(RTOS_DIR)/CMSIS_RTOS_V2
...
def generate_project(out_dir: Path, model: dict, workspace) -> Path:
    ...
    scheduler_note = (
        "Scheduler:  FreeRTOS / CMSIS-RTOS v2\n"
        "            STM32CubeF4 must include the FreeRTOS middleware\n"
        "            (Middlewares/Third_Party/FreeRTOS/Source).\n"
        if use_rtos else
        "Scheduler:  Bare-metal SysTick super-loop\n"
    )
